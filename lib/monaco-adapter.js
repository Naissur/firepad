firepad.MonacoAdapter = (function () {
  ...
  MonacoAdapter.prototype.handleModelBulkEvents = function (events) {
    ...
    // It is necessary to emit 'change' event to firepad after the monaco
    // editor selection has been changed.
    //
    // If the edit produces selection changes, the selectionChanged events will
    // first be fired from the cursor object, in the current event loop.
    //
    // So we create a one-time bulk listener, and make race between bulkListener
    // and setTimeout.


    var bulkFired = false;

    var listener = this.editor.cursor.addBulkListener(function(events) {
      for (var i = 0; i < events.length; i++) {
        var event = events[i];

        if (event.getType() === 'selectionChanged') {
          bulkFired = true;
